 ...
 */
public abstract class NYUIProgressResponseListener implements NYProgressResponseListener {
    private static final int RESPONSE_UPDATE = 0x02;

    //处理UI层的Handler子类
    private static class UIHandler extends Handler {
        //弱引用
        private final WeakReference<NYUIProgressResponseListener> mUIProgressResponseListenerWeakReference;

        public UIHandler(Looper looper, NYUIProgressResponseListener uiProgressResponseListener) {
            super(looper);
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case RESPONSE_UPDATE:
                    NYUIProgressResponseListener uiProgressResponseListener = mUIProgressResponseListenerWeakReference.get();
